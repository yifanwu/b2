...
def gen_spec(df: Table, df_name: str, config) -> Optional[ChartInfo]:
    ...
    if (df_len == 0):
        ...
        if (is_string_dtype(df[first_col]) & is_numeric_dtype(df[second_col])):
            # SATTODO: check to make sure that the length is NOT too long
            # [RULE] if there are more than 100 distinct values, give up!
...
# data based RULES
def bar_chart_bar_count_limit(df: Table, x_field: str):
    # check length
